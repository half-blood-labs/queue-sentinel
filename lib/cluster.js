...
// that has nothing to do with why something failed.
const NOISY_KEY_PATTERN = /(^|_)(id|uuid|amount|price|total|timestamp|date|time|ts)$/i;

function isNoisyField(key, value) {
  if (NOISY_KEY_PATTERN.test(key)) return true;
